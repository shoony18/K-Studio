...
class QATopViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate, UIScrollViewDelegate{
    ...
    @IBOutlet weak var QAKoukaiRange: UITextField!
    ...
    @IBOutlet weak var textValidateQAKoukaiRange: UILabel!
    ...
    override func viewDidLoad() {
        ...
        selectedRange = ["","他のユーザーに一般公開","track指定コーチに限定公開（trackプレミアム）"]
        ...
        textValidateQAKoukaiRange.isHidden = true
        ...
        QAKoukaiRange.inputView = pickerview
        QAKoukaiRange.inputAccessoryView = toolbar
...
//        print("data:\(String(describing: data))")
    ...
    @objc func done() {
        ...
        }else if currentTextField == QAKoukaiRange{
            QAKoukaiRange.endEditing(true)
            QAKoukaiRange.text = "\(selectedRange[pickerview.selectedRow(inComponent: 0)])"
            if self.QAKoukaiRange.text == "他のユーザーに一般公開"{
                self.pointExpense = "100"
            }else if self.QAKoukaiRange.text == "track指定コーチに限定公開（trackプレミアム）"{
                self.pointExpense = "500"
            }
            let ref = self.QARef.child("uuid").child("\(self.currentUid)")
    ...
    @IBAction func sendVideo(_ sender: Any) {
        ...
        if (CountQAStatusArray >= 5){
            ...
            return
        }else if QAKoukaiRange.text?.count == 0 {
            textValidateQAKoukaiRange.isHidden = false
            textValidateQAKoukaiRange.text = "公開範囲を指定してください"
            return
        }else if self.countPoint < 0{
            textValidateQAKoukaiRange.isHidden = false
            textValidateQAKoukaiRange.text = "trackポイントが\(countPoint0)pt不足してます。\n他のユーザーの質問に答えてポイントを貯めよう。"
            return
        }
            ...
            return
        }else if self.videoURL != nil && QAKoukaiRange.text == "他のユーザーに一般公開"{
            textValidateQAVideo.isHidden = false
            textValidateQAVideo.text = "指定した公開範囲では、動画の添付ができません"
            return
        }else if self.videoURL == nil && QAKoukaiRange.text == "track指定コーチに限定公開（trackプレミアム）"{
            textValidateQAVideo.isHidden = false
            textValidateQAVideo.text = "指定した公開範囲では、動画の添付が必須です"
            return
...
//        }else if self.QAKoukaiRange.text == "track指定コーチに限定公開"{
//            self.pointExpense = "50"
//        }
        ...
        let defaultAction: UIAlertAction = UIAlertAction(title: "OK", style: UIAlertAction.Style.default, handler:{
            ...
            //ここまで動画DB格納定義
            if self.QAKoukaiRange.text == "他のユーザーに一般公開"{
                self.PublicOrPrivate = "public"
            }else if self.QAKoukaiRange.text == "track指定コーチに限定公開（trackプレミアム）"{
                self.PublicOrPrivate = "private"
            }
            let fcmData = ["fcmTrigger":"1"]
//                let rangeData = "1"
            let QAdata = ["QAName":"\(timenow)\(self.currentUid)","trackAnswer":"まだコメントはありません","QASpeciality":"\(self.QASpeciality.text!)","QAContent":"\(self.QAContent.text!)","QAStatus":"QAStatus0.png","goodButton":"0","badButton":"0","date":"\(date)","time":"\(time)","uuid":"\(self.currentUid)","userName":"\(self.nameLabel.text!)","registeredUserName":"\(self.currentUserName)","PublicOrPrivate":"\(self.PublicOrPrivate!)","countAnswer":"0" as Any] as [String : Any]
                let answerdata = ["uuid":"\(self.currentUid)" as Any] as [String : Any]
                let QARef0 = self.QARef.child("\(self.currentUid)").child("\(self.PublicOrPrivate!)").child("\(timenow)\(self.currentUid)")
                let QARef1 = self.QARef.child("\(self.currentUid)").child("\(self.PublicOrPrivate!)").child("\(timenow)\(self.currentUid)").child("trackAnswer").child("\(self.currentUid)")
                let QARef2 = self.QARef.child("\(self.PublicOrPrivate!)").child("全て").child("\(timenow)\(self.currentUid)")
                let QARef3 = self.QARef.child("\(self.PublicOrPrivate!)").child("全て").child("\(timenow)\(self.currentUid)").child("trackAnswer").child("\(self.currentUid)")
                let QARef4 = self.QARef.child("\(self.PublicOrPrivate!)").child("\(self.QASpeciality.text!)").child("\(timenow)\(self.currentUid)")
                let QARef5 = self.QARef.child("\(self.PublicOrPrivate!)").child("\(self.QASpeciality.text!)").child("\(timenow)\(self.currentUid)").child("trackAnswer").child("\(self.currentUid)")
                ...
                print("\(self.textValidateQAKoukaiRange.text!)")
