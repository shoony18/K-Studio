...
class premiumSelectedMyPostViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func loadDataAnswer(){
        ...
        Ref.child("purchase").child("premium").child("uuid").child("\(self.currentUid)").child("post").child("\(self.selectedPostID!)").child("answer").child("goodTag").observeSingleEvent(of: .value, with: {(snapshot) in
